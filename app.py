...
def mom_summary(agg: pd.DataFrame, assignee: str | None) -> pd.DataFrame:
    """Month-over-month view: optionally filtered by assignee."""
    if assignee and assignee != "All":
        agg = agg[agg[ASSIGNEE_COL] == assignee].copy()
...
def run():
    ...
    assignees = ["All"] + sorted(agg[ASSIGNEE_COL].unique().tolist())
    ...
    view = mom_summary(agg, selected_assignee if selected_assignee != "All" else None)
    if month_min:
    ...
    # Summary cards (for selected assignee or overall)
    if selected_assignee != "All":
        sub = view
        label = f"**{selected_assignee}**"
    else:
    ...
    if not sub.empty:
        ...
        with c4:
            vol_ok = "Yes" if (selected_assignee == "All" or last["tickets"] >= ticket_target) else "No"
    ...
    st.subheader("Target compliance (Volume ≥ {0}, Resolution ≥ {1:.0f}%)".format(ticket_target, resolution_target * 100))
